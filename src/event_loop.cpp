...
namespace edgelink
{
    ...
    EventLoop::EventLoop()
    {
        ...
        wakeup_ = std::make_unique<Wakeup>(this);
    }
    ...
    bool EventLoop::addChannel(Channel* channel)
    {
        ...
        {
            Logger::error(
                "Failed to add channel to epoll: " + std::to_string(fd));
            return false;
        }
    ...
    bool EventLoop::removeChannel(Channel* channel)
    {
        ...
        {
            Logger::error(
                "Failed to remove channel from epoll: " + std::to_string(fd));
            return false;
        }
    ...
    void EventLoop::run()
    {
        ...
        {
            int eventCount =
                epoll_wait(epollFd_, events, maxEvents, -1);
    ...
    // 主动唤醒阻塞在 epoll_wait() 中的 EventLoop
